...
- `L2SyncPool` (requires admin)


# Example User Flow

Eg., say user _**`A`**_ is interacting with L2 chain _**`C`**_. The first entrypoint of _**`A`**_ is the `L2SyncPool` contract on _**`C`**_ ([`examples/L2/syncpools/L2ModeSyncPoolETH.sol`](/contracts//examples/L2/syncpools/L2ModeSyncPoolETH.sol))