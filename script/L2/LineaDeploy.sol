...
contract LineaDeploy is Script {
    ...
    string rpcUrl = "ttps://1rpc.io/linea";
    ...
    address l1receiver = 0x1234567890123456789012345678901234567890; // The L1 Linea Receiver address
    ...
    function run() public returns (L2Contracts memory) {
        ...
        // Deploy l2 sync pool
        {
            l2.syncPool.implementation = address(new L2LineaSyncPoolETH(endpoint));

            // Initialize the implementation for best practices
            L2LineaSyncPoolETH(l2.syncPool.implementation).initialize(
        ...
        // Set the rate parameters and token parameters
        {
            ...
            L2LineaSyncPoolETH(l2.syncPool.proxy).setMinSyncAmount(tokenIn, minSyncAmount);
            L2LineaSyncPoolETH(l2.syncPool.proxy).setL1TokenIn(tokenIn, l1TokenIn);
        }

        // Transfer the ownership to the owner if it is not the deployer
        if (owner != deployer) {
            L2LineaSyncPoolETH(l2.syncPool.proxy).transferOwnership(owner);
            L2LineaSyncPoolETH(l2.syncPool.proxy).setDelegate(owner);
