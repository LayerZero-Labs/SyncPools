...
import "../../../contracts/examples/L2/syncpools/L2LineaSyncPoolETH.sol";
import "../../../contracts/examples/L2/L2ExchangeRateProvider.sol";
import "../../../contracts/L2/syncPools/L2LineaSyncPoolETHUpgradeable.sol";
...
contract L2LineaSyncPoolETHTest is TestHelper {
    ...
    L2LineaSyncPoolETH public l2SyncPool;
    address l2ExchangeRateProvider;
    address oracle;
    address rateLimiter;
    uint256 lineaBridgeMinFee;
    ...
    function setUp() public override {
        ...
        vm.createSelectFork(LINEA.rpcUrl, LINEA.forkBlockNumber);
        ...
        l2SyncPool = L2LineaSyncPoolETH(
            _deployProxy(
                type(L2LineaSyncPoolETH).creationCode,
                abi.encode(LINEA.endpoint),
                abi.encodeCall(
        ...
        (bool s, bytes memory f) = address(LINEA.L2messenger).call(abi.encodeWithSignature("minimumFeeInWei()"));
        require(s, "minimumFeeInWei failed");

        lineaBridgeMinFee = abi.decode(f, (uint256));
    }
    ...
    function test_Token() public {
        L2LineaSyncPoolETHUpgradeable.Token memory tokenAData = l2SyncPool.getTokenData(tokenA);
        ...
        L2LineaSyncPoolETHUpgradeable.Token memory tokenBData = l2SyncPool.getTokenData(tokenB);
    ...
    function test_Messenger() public {
        assertEq(l2SyncPool.getMessenger(), LINEA.L2messenger, "test_Messenger::1");
        ...
        vm.expectEmit(true, true, true, true);
        emit MessengerSet(LINEA.L2messenger);
        l2SyncPool.setMessenger(LINEA.L2messenger);

        assertEq(l2SyncPool.getMessenger(), LINEA.L2messenger, "test_Messenger::3");
    }
    ...
    function test_Deposit() public {
        ...
        vm.expectRevert(L2LineaSyncPoolETHUpgradeable.L2LineaSyncPoolETH__OnlyETH.selector);
        l2SyncPool.deposit(tokenA, 1, 0);
        ...
        L2LineaSyncPoolETHUpgradeable.Token memory tokenAData = l2SyncPool.getTokenData(Constants.ETH_ADDRESS);
    ...
    function test_Sync() public {
        ...
        L2LineaSyncPoolETH(l2SyncPool).setPeer(ETHEREUM.originEid, bytes32(uint256(1)));

        _setUpOApp();

        MessagingFee memory fee = l2SyncPool.quoteSync(Constants.ETH_ADDRESS, "", false);

        l2SyncPool.sync{value: fee.nativeFee + lineaBridgeMinFee}(Constants.ETH_ADDRESS, "", fee);
    }
    ...
    function _setUpOApp() internal {
        ...
        L2LineaSyncPoolETH(l2SyncPool).setEnforcedOptions(enforcedOptions);

        SetConfigParam[] memory params = new SetConfigParam[](1);
        address[] memory requiredDVNs = new address[](1);
        requiredDVNs[0] = LINEA.lzDvn;
        ...
        ILayerZeroEndpointV2(LINEA.endpoint).setConfig(address(l2SyncPool), LINEA.send302, params);
    }
